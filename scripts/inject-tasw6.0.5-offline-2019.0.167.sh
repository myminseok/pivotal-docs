...
# 2) imgpkg copy -i cloudfoundry/windows2016fs:2019.0.167 --to-tar ./windows2016fs_2019.0.167.tar
...
# CURRENT_DIRECTORY/windows2016fs_2019.0.167.tar 
# CURRENT_DIRECTORY/this_script. 
# 3) cli executible: go, zip, git, bosh  note that imgpkg is not required.
# 4) edit this script for source file: tasw_tile='pas-windows-6.0.5-build.2.pivotal'
# 5) edit this script for winrootfs file: winrootfs_tar_file="windows2016fs_2019.0.167.tar"
# 6) run this script. then output will be created under current directory something like "injected-pas-windows-6.0.5-build.2.pivotal" (5GB)
...
tasw_tile='pas-windows-6.0.6-build.2.pivotal'
winrootfs_tar_file="windows2016fs_2019.0.167.tar"
...
tar xvf ./$diego_tgz -C $TMP_LOCAL_BLOB/tmp_golang
golang_tgzs=($(find $TMP_LOCAL_BLOB/tmp_golang -name "golang-*-windows.tgz"))
golang_tgz=${golang_tgzs=[0]}
cp $golang_tgz $TMP_LOCAL_BLOB/a8759efa-bd04-41c9-778f-7fbc98b3fe82
rm -rf $TMP_LOCAL_BLOB/tmp_golang
popd 



# replace meta to reuse golang-1.22-windows from pas-windows-6.0.5-build.2.pivotal instead of download from s3.
# and it should also be matched with ./.final_builds/packages/golang-1.22-windows/index.yml
cat << EOF > $TMP_TASW/embed/windowsfs-release/packages/golang-1.22-windows/spec.lock
